...
from utils.mongodb import Predictions, User, RiskStations
...
def prediction(payload: CreatePredictionSchema = Depends(), user_id: str = Depends(require_user)):
    ...
    """
    risk_stations = RiskStations.find({}, { "stations": 1, "_id": 1 }).sort("created_at", pymongo.DESCENDING).limit(1)
...
def prediction_historique(payload: HistorisationSchema = Depends(), user_id: str = Depends(require_user)):
    ...
    predictions = list(Predictions.find(query))
    prediction_array = []
